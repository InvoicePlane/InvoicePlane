...
### Security
- **Authentication Bypass via Deactivated Accounts**: `Mdl_Sessions::auth()` now rejects a login attempt for a deactivated user (`user_active != 1`) even when the supplied password is correct. Previously, disabling an account (e.g. after an employee leaves) did not actually prevent that account's credentials from being used to log in.
- **SUMEX Invoice XML Information Disclosure**: SUMEX invoice XML files (Swiss medical invoicing) are now written to a non-web-accessible `storage/temp/` directory instead of the public `uploads/temp/` folder, preventing unauthenticated access to sensitive invoice data via direct HTTP requests.
- **Access Control on `uploads/import`**: Added a `Deny from all` `.htaccess` to `uploads/import/`, the directory that holds in-progress CSV imports (`invoices.csv`, `payments.csv`, `invoice_items.csv`, ...), preventing direct web access to plaintext financial data while an import is in progress.
