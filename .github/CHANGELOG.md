...
| CSRF: GET-based quote PDF generation marks quotes sent and assigns quote numbers | Medium | — | [@ashrexon](https://github.com/ashrexon) | [#PR_PLACEHOLDER](https://github.com/InvoicePlane/InvoicePlane/pull/PR_PLACEHOLDER) |
...
- [#PR_PLACEHOLDER](https://github.com/InvoicePlane/InvoicePlane/pull/PR_PLACEHOLDER) — **CSRF (quote PDF):** `Quotes::generate_pdf()` mutated business state on a bare GET — when `mark_quotes_sent_pdf` was enabled it assigned a quote number and moved the quote from draft to sent — so a cross-site `<img>`/`<iframe>` pointing at `/quotes/generate_pdf/<id>` could silently corrupt the sales pipeline in an authenticated admin's browser. The mark-sent / number-assignment mutation now runs only on a same-origin POST carrying a valid CSRF token; a plain GET still streams the PDF but no longer changes any state. The "Download PDF" controls on the quote view, the quotes list, and the dashboard now POST the request (with the CSRF token) so the auto-mark-sent behaviour is preserved. Thanks to [@ashrexon](https://github.com/ashrexon) for the responsible disclosure.
