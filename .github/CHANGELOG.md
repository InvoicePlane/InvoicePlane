...
- [#1675](https://github.com/InvoicePlane/InvoicePlane/issues/1675) — **Stripe zero-decimal currencies:** Checkout Sessions now convert invoice balances using the configured currency's minor unit instead of multiplying every amount by 100. Callback validation, recorded payments, and merchant-response details use the same conversion, preventing JPY, KRW, CLP, and other zero-decimal invoices from charging customers 100 times the invoice total. Thanks to [@snajafov](https://github.com/snajafov) for reporting the issue.
