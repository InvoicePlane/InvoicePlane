...
class Ublv24Xml extends BaseXml
{
    ...
    protected function xmlParty(string $who)
    {
        ...
        if ( ! empty($this->options['EndpointID'])) {
            $endpointField = is_array($this->options['EndpointID'])
                ? ($this->options['EndpointID'][$who] ?? '')
                : $this->options['EndpointID'];
            $endpointProperty = $who . '_' . $endpointField;
            $endpointValue    = $this->invoice->{$endpointProperty} ?? '';

            // Peppol participant identifiers are commonly stored as "scheme:value".
            if (preg_match('/^([A-Za-z0-9]+):(.+)$/', (string) $endpointValue, $matches) === 1) {
                $schemeID      = $matches[1];
                $endpointValue = $matches[2];
            }

            $nodeEndpointID = $this->doc->createElement('cbc:EndpointID', $endpointValue);
    ...
    protected function xmlTaxSubtotal($percent, $subtotal, $category = 'S')
    {
        ...
        if ( ! empty($this->options['TaxName'])) {
            $taxName = $this->options['TaxName'][(int) $percent][0];
