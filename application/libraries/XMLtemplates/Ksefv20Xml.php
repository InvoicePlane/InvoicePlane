...
class Ksefv20Xml
{
    ...
    protected function get_invoice_totals()
    {
        ...
        if ($this->is_pl_buyer()) {
            ...
            foreach ($this->items as $item) {
                ...
                } elseif ($item->item_tax_rate_percent == 8 || $item->item_tax_rate_percent == 7) {
                    $first_reduced_net_total += $item->item_subtotal;
                    $first_reduced_tax_total += $item->item_tax_total;
                } elseif ($item->item_tax_rate_percent == 5) {
    ...
    protected function get_invoice_payment()
    {
        ...
        if (str_contains($payment_method, 'cash')) {
            $payment_method_code = 1;
        } elseif (str_contains($payment_method, 'card')) {
    ...
    protected function is_eu_country($country_code)
    {
        ...
        return (bool) (in_array($country_code, $eu_country_codes));
    }
