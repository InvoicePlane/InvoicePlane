 ...
 *
 * horst oeko zupferd by chrissie ^ x-tra-designs
 */
...
/* neu by chrissie , ACHTUNG TEST! noch nicht fertig, auch umschaltung fehlt
  von zugpferd/examples/basicsimple.php
 muss aber extendedsimple werden
*/
    public function xml()
    {
	$this->doc  = ZugferdDocumentBuilder::CreateNew(ZugferdProfiles::PROFILE_BASIC);

// $this->invoice->invoice_number

	$this->doc
	  ->setDocumentInformation("471102", "380",
	\DateTime::createFromFormat("Ymd", "20180305"),
	$this->currencyCode
	)
	  ->addDocumentNote('Rechnung ' . PHP_EOL . 'Deutschland' . PHP_EOL )
	  ...
	  ->addDocumentSellerGlobalId("4000001123452", "0088");
	    ...
	    foreach ($this->items as $index => $item) {
		    $this->doc
	   ->addNewPosition($i)
	   ->setDocumentPositionProductDetails($item->item_name)
	   ->setDocumentPositionNote($item->item_description)
	   ->setDocumentPositionGrossPrice($item->item_price)
	   ->setDocumentPositionQuantity($item->item_quantity,"");	// "" -> UnitCode
		    $i++;
