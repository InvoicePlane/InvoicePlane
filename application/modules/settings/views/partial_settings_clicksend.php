<script>
    $(function () {
        $('#clicksend_check_api_cred').click(function () {
            ...
            let apiUsername = $.trim($("#clicksend_api_username").val());
            let apiKey = $.trim($("#clicksend_api_key").val());

            // Basic client-side validation to avoid sending obviously invalid values
            if (!apiUsername || !apiKey) {
                alert("<?php echo htmlspecialchars(trans('api_credentials_required') ?? 'API username and API key are required.', ENT_QUOTES, 'UTF-8'); ?>");
                return;
            }

            $.post("<?php echo site_url('clicksend/ajax/check_api_credentials'); ?>", {api_username: apiUsername, api_key: apiKey}, function (data) {
                ...
                <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="password" name="settings[clicksend_api_key]" id="clicksend_api_key"
                                class="form-control"
                                placeholder="<?php _trans('api_key_placeholder'); ?>"
                                autocomplete="new-password">
                        </div>
                        
                        <div class="form-group">
                            <button type="button" class="btn btn-primary ajax-loader" id="clicksend_check_api_cred"><?php _trans('check_api_credentials'); ?></button>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="settings[letter_standard_color]" id="letter_standard_color"
                                class="form-control simple-select" data-minimum-results-for-search="Infinity">
                                <option value="0"><?php _trans('no'); ?></option>
                                <option value="1" <?php check_select(get_setting('letter_standard_color', true), '1'); ?>>
                                    <?php _trans('yes'); ?>
                                </option>
                            </select>
                        </div>
