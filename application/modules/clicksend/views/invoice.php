<script>
    function sanitizeInput(value) {
        ...
        // Trim whitespace and remove characters that could lead to injection issues.
    ...
    $(function () {
        $('#clicksend_check_price_btn').click(function () {
            ...
            {
                invoice_id: <?php echo (int) $invoice->invoice_id; ?>,
                address_name: sanitizeInput($("#address_name").val()),
                address_line_1: sanitizeInput($("#address_line_1").val()),
                address_line_2: sanitizeInput($("#address_line_2").val()),
                address_city: sanitizeInput($("#address_city").val()),
                address_postal_code: sanitizeInput($("#address_postal_code").val()),
                address_state: sanitizeInput($("#address_state").val()),
                address_country: sanitizeInput($("#address_country").val()),
                print_duplex: sanitizeInput($("#print_duplex").val()),
                print_color: sanitizeInput($("#print_color").val())
            },
            function (data) {
                $(".fullpage-loader-close").click();
                if (data && typeof data.total_price_format !== 'undefined') {
                    alert("<?php _trans('price'); ?>: " + data.total_price_format);
                } else {
                    alert("An unexpected response was received while retrieving the price.");
                }
            })
            .fail(function () {
                $(".fullpage-loader-close").click();
                alert("An error occurred while retrieving the price.");
            });
        });
    });
</script>
    ...
    <div id="headerbar">
        ...
        <div class="headerbar-item pull-right">
            <div class="btn-group btn-group-sm">
                <button type="button" class="btn btn-info ajax-loader" id="clicksend_check_price_btn">
            ...
            <div class="col-xs-12 col-md-8 col-md-offset-2">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="address_name" id="address_name" class="form-control" required
                        value="<?php echo html_escape($invoice->client_name); ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="address_line_1" id="address_line_1" class="form-control" required
                        value="<?php echo html_escape($invoice->client_address_1); ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="address_line_2" id="address_line_2" class="form-control"
                        value="<?php echo html_escape($invoice->client_address_2); ?>">
                </div>
                    ...
                    <div class="controls">
                        <select name="address_country" id="address_country" class="form-control simple-select">
                            <?php foreach ($countries as $cldr => $country) { ?>
                                <option value="<?php echo html_escape($cldr); ?>"
                                    <?php check_select($invoice->client_country, $cldr); ?>
                                ><?php echo html_escape($country); ?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
