...
/**
 * Check if Clicksend letter sending is configured in the settings.
 *
 * @return bool
 */
...
function clicksend_get_pdf_url($invoice)
{
    return site_url('clicksend/view/generate_invoice_pdf/' . md5(clicksend_get_auth()) . '/' . $invoice->invoice_url_key);
}
...
function clicksendapi_request($options = [])
{
    ...
    if (curl_errno($ch)) {
        ...
        return ['error' => true, 'response' => $response, 'msg' => $error];
    }
    curl_close($ch);
