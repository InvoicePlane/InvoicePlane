...
class Clicksend extends Admin_Controller
{
    ...
    public function send_invoice($invoice_id)
    {
        ...
        $invoice = $this->mdl_invoices->get_by_id($invoice_id);
        if (!$invoice) {
            $this->session->set_flashdata('alert_danger', trans('invoice_not_found'));
            redirect('invoices');
        }
        ...
        $options = [
            ...
            'body'   => [
                ...
                'duplex'        => (int) $print_duplex,
                'colour'        => (int) $print_color,
                'priority_post' => 0,
                'template_used' => 0,
                'recipients'    => [[
        ...
        if ($response['response']['response_code'] == 'SUCCESS') {
            ...
            $this->session->set_flashdata('alert_danger', html_escape($response['response']['response_msg']));
            redirect('clicksend/invoice/' . $invoice_id);
        }
    }
