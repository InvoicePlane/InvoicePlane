...
class Mdl_Reports extends CI_Model {
	...
	public function sales_by_year($from_date = NULL, $to_date = NULL, $minQuantity = NULL, $maxQuantity = NULL, $IVAchecked = False){
	...
	if($IVAchecked == FALSE){
		...
		if($maxQuantity){
			...
			$this->db->select('client_vat_id AS NIF');
			$this->db->select('client_name as Name');
			$this->db->select('(SELECT SUM(amounts.invoice_item_subtotal) FROM ip_invoice_amounts amounts WHERE amounts.invoice_id IN (SELECT inv.invoice_id FROM ip_invoices inv WHERE inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . '<= inv.invoice_date_due AND ' . $this->db->escape($to_date) . '>= inv.invoice_date_due)) AS total_payment', FALSE);
				
			for ($indice=$from_date_year;$indice<=$to_date_year;$indice++){
				$this->db->select('(SELECT SUM(invoice_item_subtotal) FROM ip_invoice_amounts WHERE invoice_id IN (select invoice_id FROM ip_invoices inv where inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . '<= inv.invoice_date_due AND ' . $this->db->escape($to_date) . '>= inv.invoice_date_due AND (inv.invoice_date_due LIKE \'%'.$indice.'-01-%\' OR inv.invoice_date_due LIKE \'%'.$indice.'-02-%\' OR inv.invoice_date_due LIKE \'%'.$indice.'-03-%\'))) AS payment_t1_'.$indice.'', FALSE);
				$this->db->select('(SELECT SUM(invoice_item_subtotal) FROM ip_invoice_amounts WHERE invoice_id IN (select invoice_id FROM ip_invoices inv where inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . '<= inv.invoice_date_due AND ' . $this->db->escape($to_date) . '>= inv.invoice_date_due AND (inv.invoice_date_due LIKE \'%'.$indice.'-04-%\' OR inv.invoice_date_due LIKE \'%'.$indice.'-05-%\' OR inv.invoice_date_due LIKE \'%'.$indice.'-06-%\'))) AS payment_t2_'.$indice.'', FALSE);
				$this->db->select('(SELECT SUM(invoice_item_subtotal) FROM ip_invoice_amounts WHERE invoice_id IN (select invoice_id FROM ip_invoices inv where inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . '<= inv.invoice_date_due AND ' . $this->db->escape($to_date) . '>= inv.invoice_date_due AND (inv.invoice_date_due LIKE \'%'.$indice.'-07-%\' OR inv.invoice_date_due LIKE \'%'.$indice.'-08-%\' OR inv.invoice_date_due LIKE \'%'.$indice.'-09-%\'))) AS payment_t3_'.$indice.'', FALSE);
				$this->db->select('(SELECT SUM(invoice_item_subtotal) FROM ip_invoice_amounts WHERE invoice_id IN (select invoice_id FROM ip_invoices inv where inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . '<= inv.invoice_date_due AND ' . $this->db->escape($to_date) . '>= inv.invoice_date_due AND (inv.invoice_date_due LIKE \'%'.$indice.'-10-%\' OR inv.invoice_date_due LIKE \'%'.$indice.'-11-%\' OR inv.invoice_date_due LIKE \'%'.$indice.'-12-%\'))) AS payment_t4_'.$indice.'', FALSE);
			}
				
			$this->db->where('(SELECT SUM(amounts.invoice_item_subtotal) FROM ip_invoice_amounts amounts WHERE amounts.invoice_id IN (SELECT inv.invoice_id FROM ip_invoices inv WHERE inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . ' <= inv.invoice_date_due AND ' . $this->db->escape($to_date) . ' >= inv.invoice_date_due AND '.$minQuantity.' <= (SELECT SUM(amounts2.invoice_item_subtotal) FROM ip_invoice_amounts amounts2 WHERE amounts2.invoice_id IN (SELECT inv2.invoice_id FROM ip_invoices inv2 WHERE inv2.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . ' <= inv2.invoice_date_due AND ' . $this->db->escape($to_date) . ' >= inv2.invoice_date_due)) AND '.$maxQuantity.' >= (SELECT SUM(amounts3.invoice_item_subtotal) FROM ip_invoice_amounts amounts3 WHERE amounts3.invoice_id IN (SELECT inv3.invoice_id FROM ip_invoices inv3 WHERE inv3.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . ' <= inv3.invoice_date_due AND ' . $this->db->escape($to_date) . ' >= inv3.invoice_date_due)))) <>0');
			...
			$this->db->where('(SELECT SUM(amounts.invoice_item_subtotal) FROM ip_invoice_amounts amounts WHERE amounts.invoice_id IN (SELECT inv.invoice_id FROM ip_invoices inv WHERE inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . ' <= inv.invoice_date_due AND ' . $this->db->escape($to_date) . ' >= inv.invoice_date_due AND '.$minQuantity.' <= (SELECT SUM(amounts2.invoice_item_subtotal) FROM ip_invoice_amounts amounts2 WHERE amounts2.invoice_id IN (SELECT inv2.invoice_id FROM ip_invoices inv2 WHERE inv2.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . ' <= inv2.invoice_date_due AND ' . $this->db->escape($to_date) . ' >= inv2.invoice_date_due)))) <>0');
		
		}
		
	} //IVAcheched == FALSE
	
	else if($IVAchecked == TRUE){
		...
		if($maxQuantity){	
				...
				$this->db->where('(SELECT SUM(amounts.invoice_total) FROM ip_invoice_amounts amounts WHERE amounts.invoice_id IN (SELECT inv.invoice_id FROM ip_invoices inv WHERE inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . ' <= inv.invoice_date_due AND ' . $this->db->escape($to_date) . ' >= inv.invoice_date_due AND '.$minQuantity.' <= (SELECT SUM(amounts2.invoice_total) FROM ip_invoice_amounts amounts2 WHERE amounts2.invoice_id IN (SELECT inv2.invoice_id FROM ip_invoices inv2 WHERE inv2.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . ' <= inv2.invoice_date_due AND ' . $this->db->escape($to_date) . ' >= inv2.invoice_date_due)) AND '.$maxQuantity.' >= (SELECT SUM(amounts3.invoice_total) FROM ip_invoice_amounts amounts3 WHERE amounts3.invoice_id IN (SELECT inv3.invoice_id FROM ip_invoices inv3 WHERE inv3.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . ' <= inv3.invoice_date_due AND ' . $this->db->escape($to_date) . ' >= inv3.invoice_date_due)))) <>0');
