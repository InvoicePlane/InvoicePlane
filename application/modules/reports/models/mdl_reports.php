		...
		if($taxChecked == FALSE){
			...
			if($maxQuantity){
				...
				for ($index=$from_date_year;$index<=$to_date_year;$index++){
					$this->db->select('(SELECT SUM(invoice_item_subtotal) FROM ip_invoice_amounts WHERE invoice_id IN (select invoice_id FROM ip_invoices inv where inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . '<= inv.invoice_date_due AND ' . $this->db->escape($to_date) . '>= inv.invoice_date_due AND (inv.invoice_date_due LIKE \'%'.$index.'-01-%\' OR inv.invoice_date_due LIKE \'%'.$index.'-02-%\' OR inv.invoice_date_due LIKE \'%'.$index.'-03-%\'))) AS payment_t1_'.$index.'', FALSE);
					$this->db->select('(SELECT SUM(invoice_item_subtotal) FROM ip_invoice_amounts WHERE invoice_id IN (select invoice_id FROM ip_invoices inv where inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . '<= inv.invoice_date_due AND ' . $this->db->escape($to_date) . '>= inv.invoice_date_due AND (inv.invoice_date_due LIKE \'%'.$index.'-04-%\' OR inv.invoice_date_due LIKE \'%'.$index.'-05-%\' OR inv.invoice_date_due LIKE \'%'.$index.'-06-%\'))) AS payment_t2_'.$index.'', FALSE);
					$this->db->select('(SELECT SUM(invoice_item_subtotal) FROM ip_invoice_amounts WHERE invoice_id IN (select invoice_id FROM ip_invoices inv where inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . '<= inv.invoice_date_due AND ' . $this->db->escape($to_date) . '>= inv.invoice_date_due AND (inv.invoice_date_due LIKE \'%'.$index.'-07-%\' OR inv.invoice_date_due LIKE \'%'.$index.'-08-%\' OR inv.invoice_date_due LIKE \'%'.$index.'-09-%\'))) AS payment_t3_'.$index.'', FALSE);
					$this->db->select('(SELECT SUM(invoice_item_subtotal) FROM ip_invoice_amounts WHERE invoice_id IN (select invoice_id FROM ip_invoices inv where inv.client_id=ip_clients.client_id AND ' . $this->db->escape($from_date) . '<= inv.invoice_date_due AND ' . $this->db->escape($to_date) . '>= inv.invoice_date_due AND (inv.invoice_date_due LIKE \'%'.$index.'-10-%\' OR inv.invoice_date_due LIKE \'%'.$index.'-11-%\' OR inv.invoice_date_due LIKE \'%'.$index.'-12-%\'))) AS payment_t4_'.$index.'', FALSE);
				}
