...
class Mdl_Item_Amounts extends CI_Model
{
    ...
    public function calculate($item_id)
    {
        ...
        // new tax total based on discounted amount
        $item_discount_total = $item->item_discount_amount * $item->item_quantity;	        //$item_tax_total = $item_subtotal * ($item->item_tax_rate_percent / 100);
        $item_total = $item_subtotal + $item_tax_total - $item_discount_total;	        $item_discount_total = ($item->item_discount_amount * $item->item_quantity) + ((($item->item_price / 100) * $item->item_discount_percent) * $item->item_quantity);
        // calculating before tax: ((subtotal - discount) + ((subtotal - discount) / 100 * tax_percent))
        // todo: give in the future option to select discount on net price / gross price
