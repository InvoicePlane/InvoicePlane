...
{
    /**
     * The scheduled endpoint. Its URL is unchanged, so existing crontab entries keep
     * working and pick up reminders automatically.
     *
     * @param string|null $cron_key
     */
    public function recur($cron_key = null)
    {
        $this->_authorize_cron_request($cron_key);

        // Reminders run first, and deliberately so. An invoice generated by the
        // recurring pass below is marked sent with a fresh due date; with a short
        // invoices_due_after it would immediately match a before-due offset and the
        // client would receive the invoice and a "payment due soon" nudge in the same
        // minute. Running reminders first makes that impossible rather than unlikely.
        $this->_process_reminders();
        $this->_process_recurring_invoices();
    }

    /**
     * Reminders only, without advancing any recurring schedule. Useful for testing the
     * reminder configuration against a live install.
     *
     * @param string|null $cron_key
     */
    public function reminders($cron_key = null)
    {
        $this->_authorize_cron_request($cron_key);
    ...
    /**
     * Rejects the request unless it carries the configured cron key. Exits rather than
     * returning on failure.
     *
     * @param string|null $cron_key
     */
    private function _authorize_cron_request($cron_key): void
    {
        ...
        if ($this->_is_cron_rate_limited()) {
            log_message('warning', '[Cron] Rate limit exceeded from: '
    ...
    /**
     * Sends any payment reminders that are due. Isolated from the recurring pass so a
     * fault here can never stop invoices being generated.
     */
    private function _process_reminders(): void
    {
        if ( ! get_setting('invoice_reminders_enabled')) {
            if (IP_DEBUG) {
                log_message('debug', '[Cron Invoice Reminders] Disabled in the settings, skipping');
            }
        ...
        if ( ! mailer_configured()) {
            log_message('error', '[Cron Invoice Reminders] Mailer is not configured, skipping');
    ...
    /**
     * Generates and emails invoices from the recurring schedules that are due.
     */
    private function _process_recurring_invoices(): void
    {
        $this->load->model([
