				...
				<div class="row">
						...
						<div class="row">
							<div class="col-xs-12 col-sm-1">
								...
								<?php if ($invoice->is_read_only != 1): ?>
									<button type="button" class="btn_delete_item btn btn-link" title="<?php _trans('delete'); ?>" data-item-id="<?=$item->item_id;?>">
										<i class="fa fa-trash-o text-danger"></i>
									</button>
								<?php endif; ?>
							</div>
							...
							<div class="col-xs-12 col-sm-11">
								...
								<div class="input-group">
									<label for="item_name_<?=$item->item_id;?>" class="input-group-addon ig-addon-aligned"><?php _trans('item'); ?></label>
									<input type="text" name="item_name" id="item_name_<?=$item->item_id;?>" class="input-sm form-control" value="<?=_htmlsc($item->item_name);?>"<?=$invoice_disabled;?>>
								</div>
								<?php if ($invoice->sumex_id == "") : ?>
									<div class="input-group">
										<label for="item_description_<?=$item->item_id;?>" class="input-group-addon ig-addon-aligned"><?php _trans('description'); ?></label>
										<textarea name="item_description" id="item_description_<?=$item->item_id;?>" class="input-sm form-control h135rem"<?=$invoice_disabled;?>><?=htmlsc($item->item_description);?></textarea>
									</div>
								<?php else: ?>
									<div class="input-group">
										<label for="item_date_<?=$item->item_id;?>" class="input-group-addon ig-addon-aligned"><?php _trans('date'); ?></label>
										<input type="text" name="item_date" id="item_date_<?=$item->item_id;?>" class="input-sm form-control datepicker" value="<?php echo format_date($item->item_date);?>"<?=$invoice_disabled;?>>
									</div>
								<?php endif; ?>
							</div>
						</div>
					...
					<div class="col-xs-12 col-sm-5 col-md-6 col-lg-7">
						<div class="row">
							<div class="col-xs-12 col-lg-6">
								<div class="input-group">
									<label for="item_quantity_<?=$item->item_id;?>" class="input-group-addon ig-addon-aligned"><?php _trans('quantity'); ?></label>
									<input type="text" name="item_quantity" id="item_quantity_<?=$item->item_id;?>" class="input-sm form-control" value="<?=format_amount($item->item_quantity);?>"<?=$invoice_disabled;?>>
								</div>
								<div class="input-group">
									<label for="item_product_unit_id_<?=$item->item_id;?>" class="input-group-addon ig-addon-aligned"><?php _trans('product_unit'); ?></label>
									<select name="item_product_unit_id" id="item_product_unit_id_<?=$item->item_id;?>" class="form-control input-sm"<?=$invoice_disabled;?>>
										<option value="0"><?php _trans('none'); ?></option>
										<?php foreach ($units as $unit) { ?>
											<option value="<?=$unit->unit_id;?>"
								...
								<div class="input-group">
									<label for="item_price_<?=$item->item_id;?>" class="input-group-addon ig-addon-aligned"><?php _trans('price'); ?></label>
									<input type="text" name="item_price" id="item_price_<?=$item->item_id;?>" class="input-sm form-control"
										   value="<?php echo format_amount($item->item_price); ?>"<?=$invoice_disabled;?>>
										<div class="input-group-addon"><?php echo get_setting('currency_symbol'); ?></div>
								</div>
								<div class="input-group">
									<label for="item_discount_amount_<?=$item->item_id;?>" class="input-group-addon ig-addon-aligned"><?php _trans('item_discount'); ?></label>
									<input type="text" name="item_discount_amount" id="item_discount_amount_<?=$item->item_id;?>" class="input-sm form-control"
								...
								<div class="input-group">
									<label for="item_tax_rate_id_<?=$item->item_id;?>" class="input-group-addon ig-addon-aligned"><?php _trans('tax_rate'); ?></label>
									<select name="item_tax_rate_id" id="item_tax_rate_id_<?=$item->item_id;?>" class="form-control input-sm"<?=$invoice_disabled;?>>
										<option value="0"><?php _trans('none'); ?></option>
										<?php foreach ($tax_rates as $tax_rate) { ?>
											<option value="<?=$tax_rate->tax_rate_id;?>"
